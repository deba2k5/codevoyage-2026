...
export default function Home() {
  ...
  const [chosenCharacterId, setChosenCharacterId] = useState<string>(characters[0].id);
  ...
  const chosenCharacter = characters.find(c => c.id === chosenCharacterId) || characters[0];

  const scrollToCharacterDetail = () => {
    document.querySelector('#character')?.scrollIntoView({ behavior: 'smooth', block: 'start' });
  };

  const handleChooseHero = () => {
    setChosenCharacterId(activeCharacter.id);
    scrollToCharacterDetail();
  };
